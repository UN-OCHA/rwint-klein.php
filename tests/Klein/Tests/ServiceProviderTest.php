...
class ServiceProviderTest extends AbstractKleinTest {
  ...
  public function testRefresh() {
    ...
    $this->assertSame(
      $this->kleinApp->request()->uri(),
      $this->kleinApp->response()->headers()->get('location')
    );
    $this->assertTrue($this->kleinApp->response()->isLocked());

    // Make sure we got a 3xx response code.
    $this->assertGreaterThan(299, $this->kleinApp->response()->code());
    $this->assertLessThan(400, $this->kleinApp->response()->code());
  }
  ...
  public function testRenderChunked() {
    ...
    $this->kleinApp->dispatch(NULL, $response);
